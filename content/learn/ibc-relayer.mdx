...
## Running relayer with SHE

### Installation

We will be using hermes as example relayer. Please note, that SHE requires
...
key_name = 'wallet'
store_prefix = 'ibc'
gas_price = { price = 0.025, denom = 'ushe' }
max_gas = 10000000
gas_multiplier = 2
clock_drift = '5s'
trusting_period = '14days'
...
```bash copy
hermes keys add --key-name wallet --chain she-chain --mnemonic-file ~/misc/mm_file_she
hermes keys add --key-name wallet --chain ibc-0 --mnemonic-file ~/misc/mm_file_ibc_0
```

The contents of ~/misc/mm*file*\* file is just a seed phrase of the chain
...
```bash copy
hermes create channel --a-chain she-chain --b-chain ibc-0 --a-port transfer --b-port transfer --new-client-connection
```
