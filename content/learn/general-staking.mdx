...
Staking is a fundamental aspect of the SHE blockchain, enabling network security
...
growth of the SHE ecosystem and participate in [governance](/general-governance)
...
The SHE protocol incentivizes validators and delegators with staking rewards
...
Users can view a validator’s commission rate on the SHE App, or by querying the validator information on-chain.

## How to Stake

Staking on SHE can be done through the SHE App, via the `shed` CLI, or even
through API calls.

- **SHE App**: [Staking](https://app.she.io/stake)
- **shed CLI**: `shed tx staking [command] [flags]`
...
Delegators are users who stake their SHE tokens to a validator, contributing to
the validator’s total stake. Delegators earn a portion of the transaction fees
as staking rewards. Staked SHE remains under the control of the delegator and
...
Bonding is the process of committing your SHE tokens to a validator. This can be
...
Delegators can un-bond or un-stake their SHE tokens, initiating a 21-day period
...
- **Double Signing**: Signing two different blocks with the same chain ID at the
  same height.
...
They can be queried through the [slashing module](https://rest.she-apis.com/cosmos/slashing/v1beta1/params).
...
Staking rewards are calculated based on the total amount of SHE staked and the validator’s performance. Rewards are distributed proportionally to the amount staked by each delegator.
