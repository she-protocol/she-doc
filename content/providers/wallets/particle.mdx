...
and wallet adapters (such as MetaMask) fully compatible with SHE, enabling
2-click onboarding.
...
2. Execute a **gasless (sponsored) transaction**, showcasing the SDK's
   capability to simplify user interactions on the SHE blockchain.

## Getting started

### Dependencies

To integrate Particle Connect into your SHE application, you'll need just a few
...
import { she, sheTestnet } from "@particle-network/connectkit/chains";
...
authentication and blockchain interactions on SHE and SHE testnet. It includes
...
2. **Funding Requirements**: On a Testnet like SHE, all transactions are
   automatically sponsored, meaning you don't need to deposit USDT to cover
   transaction fees. However, on mainnets like SHE, the Paymaster (configurable
...
In this example, the application creates a smart account on SHE using a social
login or a Web3 login and sends a gasless transaction of 0.01 SHE via the
`ethers` provider.
...
Building an application on SHE that takes advantage of social logins and smart
...
[GitHub repository](https://github.com/Particle-Network/she-aa-connect).
