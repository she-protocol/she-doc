...
- `send`: Enables a user to send non-native tokens to another wallet. For
  sending native SHE tokens, use `sendNative` instead.

  ```solidity copy
    /// Sends non-native tokens from one address to another.
    /// @param fromAddress The SHE address to send funds from.
    /// @param toAddress The SHE address to send funds to.
...
- `sendNative`: Enables a user to send native SHE to another user.
  ```solidity copy
    /// Sends SHE to the specified user..
    /// @param toNativeAddress The SHE address of the recipient.
...
- `balance`: Queries the balance of the given account for specified denom.
  ```solidity copy
    ...
    /// @param acc The SHE address of the account to query.
...
```solidity copy
    ...
    /// @param acc The SHE address of the account to query.
    /// @return balances for all coins/denoms.
...
<Callout type="info">View the Bank precompile source code and the contract ABI [here](https://github.com/she-protocol/she-chain/tree/main/precompiles/bank).</Callout>
