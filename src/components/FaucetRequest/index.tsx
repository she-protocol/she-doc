...
const VITE_FAUCET_API_URL = 'https://staging-faucet-v3.shenetwork.io';
...
const RequestFaucetCard = () => {
	...
	const isValidAddress = isValidSheCosmosAddress(destAddress) || isAddress(destAddress);
	...
	const handleSubmit = async () => {
		...
		if (!isValidAddress) {
			toast.error('Invalid SHE or EVM address');
			return;
		}
	...
	return (
		<div className='w-full text-neutral-900 dark:text-neutral-100 flex flex-col items-center gap-6'>
			...
			<Flex align='center' className='w-full gap-4'>
				...
				<div>
					<h2 className='text-2xl md:text-3xl font-bold'>SHE Faucet</h2>
					<p className='text-sm md:text-xl opacity-80'>Request 5 SHE every 24 hours for testing.</p>
				</div>
			</Flex>
			...
			<div className='w-full flex items-center border border-neutral-300 dark:border-neutral-600 rounded-md p-2 focus-within:ring-2 focus-within:ring-neutral-400 dark:focus-within:ring-neutral-500'>
				<input
					...
					placeholder='SHE or EVM address...'
					value={destAddress}
			...
			{nextUseTime && (
				<div className='flex items-center gap-3 p-4 border-l-4 border-red-500 bg-red-100 text-red-800 rounded w-full'>
					...
					<p className='text-lg font-medium'>You can request SHE testnet tokens again after {nextUseTime}.</p>
				</div>
			)}
			...
			<Flex direction='column' className='w-full gap-5'>
				...
				{txHash && (
					<Button asChild variant='ghost' className='flex items-center gap-2 w-full !p-6 !rounded-2xl'>
						<a href={`https://www.shestream.app/transactions/${txHash}?chain=${selectedChain}`} target='_blank' rel='noopener noreferrer'>
							<IconWorld className='w-6 h-6' />
							View Transaction on SheStream
						</a>
					</Button>
				)}
			</Flex>
